...
def train(S,A,returns):
    ...
    for i in range(POLICY_TRAIN_ITERS):
        ...
        # objective = -log_probs * returns.to(DEVICE)
        # objective = 0
        # for i in range(n):
        ...
        # print(objective)
        # print('log_probs * returns[n]')
        # print(log_probs * returns[n])
        # objective = -torch.sum(log_probs * returns[n])
        objective.backward()
        OPT.step()
...
if "mountain_car" in args.mode:
    ...
    print("He Zuwang's program has been finished")
